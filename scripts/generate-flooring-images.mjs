 ...
 * Run with: node scripts/generate-flooring-images.mjs
 */
...
const OUTPUT_DIR = path.join(__dirname, "..", "public", "images", CATEGORY);
...
async function generateImage(product) {
  try {
    ...
    const imageParts = result.response.candidates?.[0]?.content?.parts?.filter((p) => p.inlineData?.mimeType?.startsWith('image/')) || [];
    ...
    console.error(`Error generating ${product.name}:`, e.message || e); 
    return null; 
  }
}
